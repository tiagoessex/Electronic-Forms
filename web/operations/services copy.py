...
from idrisk.settings import MEDIA_URL, STATIC_ROOT 
...
MESSAGE_NO_VALUE = {'code':2, 'message': 'No value!'}
MESSAGE_INCORRECT_FORMAT = {'code':3, 'message': 'Incorrect format/pattern!'} # PATTERN
MESSAGE_NO_UPPERCASE = {'code':4, 'message': 'No uppercase!'}
MESSAGE_LENGTH_EXCEEED = {'code':5, 'message': 'Max length exceed!'}
MESSAGE_MAX_EXCEEDED = {'code':6, 'message': 'Max value exceeded!'}
MESSAGE_MIN_EXCEEDED = {'code':7, 'message': 'Min value exceeded!'}
MESSAGE_MIN_EXCEEDED = {'code':8, 'message': 'Min value exceeded!'}
MESSAGE_NOTHING_SELECTED = {'code':9, 'message': 'No option selected!'}


def removeUnusedAssets(operation_id):
    """
    ...
    """
    try:
        data = OperationData.objects.get(operation=operation_id)#.values_list('data')

        # to keep
        in_use = []
        if data.data and 'elements' in data.data:
            for element in data.data.get('elements'):
                getRelativeUrls(element, in_use)
            if 'annexes' in data.data:
                in_use += data.data.get('annexes')
            assets_to_keep = []
            for asset_in_use in in_use:
                ...
                try:
                    ...
                    print("OperationData [" + str(data.id) + "]: Asset: [" + filename + "] does not exist!")
            ...
            if (element_type not in ['CHECKBOX','RADIO']):
                ...
                for gp in groups:
                    if gp.get('id') == group:
                        ...
                        gp['required'] = True if gp['required']=='yes' or gp['required'] else False
