...
export class BarcodeTextElement  extends FormBaseElement {
    constructor(context, props, id) {
        ...
        $(this.btn.dom).on('click', function() {
            context.signals.onBarcodeReader.dispatch((code, image) => {
                ref.input.setValue(code);
                context.signals.onFormValueChanged.dispatch(ref.real_id, code); 
            }, error => {
                context.signals.onError.dispatch(error,"[BarcodeTextElement::ctor]");
            }); 
        })
