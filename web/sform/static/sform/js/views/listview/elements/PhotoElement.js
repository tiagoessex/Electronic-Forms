...
import { PHOTO_INPUT } from '/static/js/urls.js';
...
export class PhotoElement extends ListBaseElement {
    constructor(context, props, id) {
        ...
        this.img = new Img(PHOTO_INPUT).attachTo(this.body);
        ...
        $(this.button.dom).on('click',function() {
            $(ref.img.dom).attr('src',PHOTO_INPUT);
            ref.button.removeClass('d-block');
            context.signals.onListValueChanged.dispatch(ref.real_id, PHOTO_INPUT, true); 
         });
        ...
        $(this.input.dom).on('change',  (ev) => {
            ...
            if (ev.target.files && ev.target.files[0]) {
                ...
                UploadAsset(
                    ...
                    (error) => {
                        context.signals.onError.dispatch(error,"[PhotoElement::ctor]");
                    }
                );
            }
        });
        ...
        context.signals.onFormValueChanged.add((id, value, reset) => {
            if (id === this.real_id) {
                reset?this.button.removeClass('d-block'):this.button.addClass('d-block');
    ...
    save() {
        ...
        file !== getFileFromUrl(PHOTO_INPUT)?(this.status.value = file): (this.status.value = null);
    ...
    async restore(data) {
        ...
        if (this.status.value) {
            ...
            */
            ...
            $(this.img.dom).attr('src',PHOTO_INPUT);
        }
