...
import { SelectionActionRow } from './SelectionActionRow.js';
...
/**
 * Selection Action Card for Basic E/A.
 */
export class SelectionActionCard extends ActionCard {
    ...
    constructor(context, id, data_restore) {
        ...
        this.action_selector = new ActionSelector(context, null, EA_TYPE.SELECTION).attachTo(col_action);
        if (data_restore) this.action_selector.setValue(data_restore.actions.action);

        // initial row
        this.rows.push(new SelectionActionRow(context, this.onAddRow, this.onRemoveRow, data_restore?data_restore.actions.target_fields[0]:null).attachTo(this.col_fields));

        if (data_restore && data_restore.actions) this.restore(data_restore.actions);

        add_btn.dom.addEventListener('click',() => {
            this.rows.push(new SelectionActionRow(this.context, this.onAddRow, this.onRemoveRow).attachTo(this.col_fields));
     ...
     * @param {SelectionActionRow} row SelectionActionRow to be saved.
     */
     onAddRow = (row) =>  {
        this.rows.push(row);
    }
     ...
     * @param {SelectionActionRow} row SelectionActionRow to be removed.
     */
    onRemoveRow = (row) => {
        row.clear();
    ...
    restore(data) {      
        data.target_fields.forEach((row, index) => {
            if (index != 0) {
                const ea = new SelectionActionRow(this.context, this.onAddRow, this.onRemoveRow, row).attachTo(this.col_fields);
                this.rows.push(ea);
            }
        })
    }
