...
import { TemporalActionRow } from './TemporalActionRow.js';
...
/**
 * Temporal Card for Auto E/A.
 */
export class TemporalActionCard extends ActionCard {
    ...
    constructor(context, id, data_restore) {
        ...
        this.action_selector = new ActionSelector(context, null, EA_TYPE.TEMPORAL).attachTo(col_action);
        if (data_restore) this.action_selector.setValue(data_restore.actions.action);

        this.rows.push(new TemporalActionRow(context, this.onAddRow, this.onRemoveRow, data_restore?data_restore.actions.target_fields[0]:null).attachTo(this.col_fields));

        if (data_restore && data_restore.actions) this.restore(data_restore.actions);

        add_btn.dom.addEventListener('click',() => {
            this.rows.push(new TemporalActionRow(this.context, this.onAddRow, this.onRemoveRow).attachTo(this.col_fields));
     ...
     * @param {TemporalActionRow} row TemporalActionRow to be saved.
     */
    onAddRow = (row) =>  {
        this.rows.push(row);
    }
     ...
     * @param {TemporalActionRow} row TemporalActionRow to be removed.
     */
    onRemoveRow = (row) => {
        self.clear();
    ...
    restore(data) {
        data.target_fields.forEach((row, index) => {
            if (index != 0) {
                const ea = new TemporalActionRow(this.context, this.onAddRow, this.onRemoveRow, row).attachTo(this.col_fields);
                this.rows.push(ea);
            }
        })
    }    
