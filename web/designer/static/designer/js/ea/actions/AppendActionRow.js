...
/**
 * Append E/A Action Row.
 */
 export class AppendActionRow extends ActionRow {
    ...
    constructor(context, onAdd, onRemove, data_restore=null) {
        ...
        const col_fields = new Div({classes:['col-9']}).attachTo(this);
        this.field_selector = new EAElementSelector(context, null, EA_TYPE.APPEND.elements_allowed.actions).attachTo(col_fields);
        if (data_restore) this.field_selector.setValue(data_restore).checkStatus();

        
        const col_ops = new Div({classes:['col-3','align-middle','text-center']}).attachTo(this);
        ...
        this.delete_field.dom.addEventListener('click', function (e) {
            ...
            // remove selectors before signal
        ...
        this.add_field.dom.addEventListener('click', function (e) {
            ...
            const new_row = new AppendActionRow(self.context, onAdd, onRemove);            
            onAdd(new_row);
            if (anchor)
