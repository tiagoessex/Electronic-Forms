 ...
 * Radio element.
 * 
 * Notes : The label adjusts according to the circle size.
 * 
 */
export class RadioElement extends BaseElement {
    ...
    constructor(type=ELEMENTS_TYPE.NONE, props = null, parent=null, context=null, left, top, id, background=null) {
        ...
        this.setStyle("-webkit-border-radius",(props?props[PROPERTIES_ID.BORDERRADIUSPROPERTY]:"75") + 'px');
        this.setStyle("-moz-border-radius",(props?props[PROPERTIES_ID.BORDERRADIUSPROPERTY]:"75") + 'px');
        this.setStyle("border-radius",(props?props[PROPERTIES_ID.BORDERRADIUSPROPERTY]:"75") + 'px');
        ...
        if (props) {
            ...
            if (props[PROPERTIES_ID.SHOWLABELPROPERTY] === 'yes') {
                this.label.setTextContent(props[PROPERTIES_ID.LABELPROPERTY] !==''?props[PROPERTIES_ID.LABELPROPERTY]:id);//'Radio');
            } else {
        ...
        this.signal_onElementResized = context.signals.onElementResized.add((element, width, height) => {
            ...
            // horizontal
            //label.style.left = (parseInt(this.dom.style.width) + 4) + 'px';
            label.style.left = (parseInt(width) + 4) + 'px';
            // vertically
            let font_size = parseInt(this.dom.style.fontSize);
            if (font_size !== font_size)    // check if nan
                font_size = DEFAULT_FONT_SIZE;
            //label.style.top = (parseInt(this.dom.style.height) / 2 - font_size) + 'px';
